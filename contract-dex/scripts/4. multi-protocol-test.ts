...
async function main() {
  ...
  try {
    ...
    console.log(`📊 초기 분배 비율: AAVE ${Number(aaveAlloc)/100}%, Morpho ${Number(morphoAlloc)/100}%`);
    ...
    console.log(`📊 변경된 분배 비율: AAVE ${Number(aaveAlloc)/100}%, Morpho ${Number(morphoAlloc)/100}%`);
    ...
    // 사용자 정보 확인
    const [userShares, userAssets, shareValue] = await vault.getUserInfo(deployer.address, USDT_ADDRESS);
    ...
    // Share 가치 변화 확인
    const [, newUserAssets, newShareValue] = await vault.getUserInfo(deployer.address, USDT_ADDRESS);
    ...
    if (totalBalance3 > 0) {
      ...
      console.log(`📈 리밸런싱 후 실제 비율: AAVE ${Number(newAavePerc)/100}%, Morpho ${Number(newMorphoPerc)/100}%`);
    }
    ...
    // 최종 사용자 정보
    const [finalShares, finalAssets] = await vault.getUserInfo(deployer.address, USDT_ADDRESS);
    console.log(`\n📊 최종 사용자 보유 현황:`);
    ...
    const [deposited, withdrawn, totalSharesSupply, currentValue] = await vault.getTokenStats(USDT_ADDRESS);
    ...
    if (netDeposited > 0) {
      const totalReturn = ((Number(netValue) - Number(netDeposited)) / Number(netDeposited)) * 100;
    ...
    await bridgeTx.wait();
    console.log(`✅ 브릿지 방식으로 ${ethers.formatUnits(bridgeAmount, 6)} USDT 예치 완료`);
