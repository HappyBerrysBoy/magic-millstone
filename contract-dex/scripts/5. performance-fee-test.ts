...
async function main() {
  ...
  try {
    ...
    console.log('\n=== 1. CompactMultiProtocolVault 배포 ===');
    
    const VaultFactory = await ethers.getContractFactory('CompactMultiProtocolVault');
    const implementation = await VaultFactory.deploy();
    await implementation.waitForDeployment();
    const implementationAddress = await implementation.getAddress();
    console.log('✅ CompactMultiProtocolVault 구현체 배포:', implementationAddress);
    ...
    const vault = await ethers.getContractAt('CompactMultiProtocolVault', proxyAddress);
    ...
    // 초기 상태 확인
    let [shares, assets, shareValue] = await vault.getUserInfo(deployer.address, USDT_ADDRESS);
    ...
    if (totalProtocolBalance > 0) {
      ...
      console.log(`📈 실제 분배 비율: AAVE ${Number(actualAavePerc)/100}%, Morpho ${Number(actualMorphoPerc)/100}%`);
    }
    ...
    console.log(`💰 수집된 Performance Fee: ${ethers.formatUnits(currentAccumulatedFees, 6)} USDT`);
    ...
    // 총 수익률 (Fee 제외 전)
    const [totalValue, principal, grossYield, yieldRate] = await vault.calculateYield(USDT_ADDRESS);
    ...
    // 순 수익률 (Fee 제외 후)
    const [, , netGrossYield, feeAmount, netYield, netYieldRate] = await vault.calculateNetYield(USDT_ADDRESS);
    console.log(`\n📊 사용자 순 수익률 분석:`);
    ...
    console.log(`📋 Fee 인출 전 수취인 잔액: ${ethers.formatUnits(feeRecipientBalanceBefore, 6)} USDT`);
    ...
    if (accumulatedFees > 0) {
      ...
      console.log(`📋 Fee 인출 후 수취인 잔액: ${ethers.formatUnits(feeRecipientBalanceAfter, 6)} USDT`);
      console.log(`💰 실제 인출된 Fee: ${ethers.formatUnits(feeRecipientBalanceAfter - feeRecipientBalanceBefore, 6)} USDT`);
    } else {
    ...
    // 사용자 현재 정보
    [shares, assets, shareValue] = await vault.getUserInfo(deployer.address, USDT_ADDRESS);
    ...
    console.log(`💵 실제 출금 받은 금액: ${ethers.formatUnits(userBalanceAfter - userBalanceBefore, 6)} USDT`);
    ...
    // 프로토콜별 잔액
    const [finalAaveBalance3, finalMorphoBalance3, finalTotalBalance3] = await vault.getProtocolBalances(USDT_ADDRESS);
    console.log(`\n📊 프로토콜별 최종 잔액:`);
    ...
    const [finalAaveBalance, finalMorphoBalance, finalTotalBalance] = await vault.getProtocolBalances(USDT_ADDRESS);
    console.log(`📊 분산 투자 최종 결과:`);
    console.log(`  💼 AAVE 최종 잔액: ${ethers.formatUnits(finalAaveBalance, 6)} USDT`);
    console.log(`  🥩 Morpho 최종 잔액: ${ethers.formatUnits(finalMorphoBalance, 6)} USDT`);
    ...
    console.log(`\n📈 분산 투자 효과 (전체 기간):`);
    console.log(`  🔄 최종 위험 분산: AAVE ${(aavePortion*100).toFixed(1)}% + Morpho ${(morphoPortion*100).toFixed(1)}%`);
    console.log(`  💰 총 벌어들인 수익: ${ethers.formatUnits(totalProfitEarned, 6)} USDT`);
    console.log(`  📊 30일 수익률: ${(Number(totalProfitEarned) / 1001 * 100).toFixed(4)}%`);
    console.log(`  📈 연환산 수익률: ${(Number(totalProfitEarned) / 1001 * (365/30) * 100).toFixed(4)}%`);

    console.log('\n=== 12. Performance Fee 효과 분석 ===');
    
    const originalDeposit = 1001; // 1000 + 1 USDT
    const finalUserValue = Number(ethers.formatUnits(userBalanceAfter - userBalanceBefore + assets, 6));
    ...
    console.log(`  📈 사용자 순 수익률: ${((finalUserValue - originalDeposit) / originalDeposit * 100).toFixed(4)}%`);
    console.log(`  💎 Fee 수익률: ${(totalFeeCollected / originalDeposit * 100).toFixed(4)}%`);
