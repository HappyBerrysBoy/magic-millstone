...
import { mm_apy_hsts as _mm_apy_hsts } from './mm_apy_hsts';
import type {
  mm_apy_hstsAttributes,
  mm_apy_hstsCreationAttributes,
} from './mm_apy_hsts';
...
export function initModels(sequelize: Sequelize) {
  const mm_apy_hsts = _mm_apy_hsts.initModel(sequelize);
  const mm_exchange_rate_hsts = _mm_exchange_rate_hsts.initModel(sequelize);
  const mm_globals = _mm_globals.initModel(sequelize);
  const mm_portfolio_allocations =
    _mm_portfolio_allocations.initModel(sequelize);
...
export interface millstoneModels {
  mm_portfolio_apy_hsts: any;

  mm_apy_hsts: typeof _mm_apy_hsts;
